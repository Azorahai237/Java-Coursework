 ...
 *
 * @author Nick Efford & Ting Hang Nathan Lam
 */
...
public class Point {
  ...
  public Point(ZonedDateTime t,double lon,double lat,double elev) throws GPSException{
    //setting parameters to class values
    this.time = t;
    this.longitude = lon;
    this.latitude = lat;
    this.elevation = elev;
    ...
    if (this.longitude < MIN_LONGITUDE || this.longitude > MAX_LONGITUDE){
      throw new GPSException("Longitude provided is invalid.");
    }
    if (this.latitude < MIN_LATITUDE || this.latitude > MAX_LATITUDE){
      throw new GPSException("Latitude provided is invalid");
    }
  }
  ...
  @Override public String toString(){
    String output = String.format("(%.5f, %.5f), %.1f m",this.longitude, this.latitude, this.elevation);
    return output;
  }
